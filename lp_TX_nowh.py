...
class LPSolver:
    def __init__(self, data_folder: str):
        ...
        self._combine_forecast()  # prophet_TX_i_df를 합쳐서 self.D 생성
        ...
        # ─── 7) 운송 용량 상수 정의 ───
        #    facility-origin만 존재하므로 U_wh를 사용
    ...
    def _combine_forecast(self):
        """
        prophet_TX_1_df ~ prophet_TX_3_df를 하나의 MultiIndex 구조로 합쳐서 self.D 생성
    ...
    def _load_specific_holding_costs(self):
        ...
        for fname in file_list:
            ...
            # CSV: store_id, item_id, 2017(holding cost) 컬럼 가정
    ...
    def _load_distance_info(self):
        ...
        for _, row in df_dist.iterrows():
            state = row["STATE"]
            d_fcst = float(row["FCST"])  # facility→store 거리
    ...
    def _build_constraints(self):
        """
        제약조건 구현:
        (T1) Transportation capacity  → 부피 및 U_wh 반영 (Wh 제거)
        ...
        # (T1) Transportation capacity ⟶ “부피 합 ≤ U_wh × N”
        for n1, n2 in arcs:
            for t in periods:
                expr = gp.LinExpr()
                for i in all_items:
                ...
                # 모든 facility-origin에 대해 U_wh 사용
                self.model.addConstr(
                    expr <= self.U_wh * self.N[n1, n2, t],
                    name=f"T1_vol_U_{n1}_{n2}_{t}",
                )
...
if __name__ == "__main__":
    ...
    # 4) “TX_wh” 노드(창고)가 모델에 포함되었는지 확인
    wh_vars = [v.varName for v in solver.model.getVars() if "TX_wh" in v.varName]
    wh_constrs = [
        c.ConstrName for c in solver.model.getConstrs() if "TX_wh" in c.ConstrName
    ]

    print("TX_wh 관련 변수:", wh_vars)
    print("TX_wh 관련 제약:", wh_constrs)
