...
class LPSolver:
    def __init__(self, data_folder: str):
        ...
        self._combine_forecast()  # prophet_WI_i_df 세 개를 합쳐서 self.D 생성

        # ─── 3) 노드·아이템별 보관비 불러오기 ───
        self._load_specific_holding_costs()

        # ─── 4) 품목별 부피 정보 불러오기 (transportation_capacity.csv) ───
        self._load_volume_info()

        # ─── 5) 거리 정보 불러오기 (distance.csv) ───
        self._load_distance_info()

        # ─── 6) 카테고리별 운송비 계수 불러오기 (operation_cost_by_category.csv) ───
        self._load_category_costs()

        # ─── 7) 운송 용량 상수 정의 ───
        #    store-origin일 때, U_store; facility-origin일 때, U_wh
        self.U_store = 5943756.325
        self.U_wh = 41606294.28
    ...
    def _read_csv(self, filename):
        ...
        test_data/demand_forecast/jw_prophet에서 파일을 읽어옵니다.
        """
        if filename in [
    ...
    def _load_data(self):
        ...
        # ───── 예측 데이터(WI) 불러오기 ─────
        self.prophet_WI_1_df = self._read_csv("jw_WI_1.csv")
        self.prophet_WI_2_df = self._read_csv("jw_WI_2.csv")
        self.prophet_WI_3_df = self._read_csv("jw_WI_3.csv")

    def _combine_forecast(self):
        """
        prophet_WI_1_df ~ prophet_WI_3_df를 하나의 MultiIndex 구조로 합쳐서 self.D 생성
        index: (item_name, n, t), value: demand
        """
        dfs_and_nodes = [
            (self.prophet_WI_1_df, "WI_1"),
            (self.prophet_WI_2_df, "WI_2"),
            (self.prophet_WI_3_df, "WI_3"),
        ]
    ...
    def _load_specific_holding_costs(self):
        ...
        (item_id, store_id) → holding cost 값을 self.holding_costs 딕셔너리에 저장.
        """
        file_list = [
    ...
    def _load_volume_info(self):
        """
        transportation_capacity.csv를 읽어서,
        - 각 CATEGORY 당 VOLUMEPERITEM,
        - 전체 VOLUME 한계 (예: 42000)
        을 self.volume_per_item, self.volume_capacity에 저장.
        """
        ...
        # 형태: CATEGORY, VOLUMEPERITEM, VOLUME
        # 예:
        #   FOODS,1,42000
        #   HOBBIES,2,42000
        #   HOUSEHOLD,6,42000

        # 1) 전체 capacity(모든 행 동일 가정)
    ...
    def _load_distance_info(self):
        """
        distance.csv를 읽어서,
        - STATE별로 FCST 거리만 사용 (창고 Wh 제거)
        → self.distance_map[(n1, n2)] 형태로 저장.
        """
        df_dist = self.distance_df.copy()
        # 열: STATE, FCWH, FCST, WHST (Wh 관련은 무시)
        self.distance_map = {}

        for _, row in df_dist.iterrows():
            state = row["STATE"]
            d_fcst = float(row["FCST"])  # 생산지→판매지 거리

            fac_node = f"{state}_fac"
            st_nodes = [f"{state}_1", f"{state}_2", f"{state}_3"]
    ...
    def _load_category_costs(self):
        """
        operation_cost_by_category.csv를 읽어서,
        - 각 CATEGORY별 COEFFICIENT(α)와 INTERCEPT(β)을
          self.category_costs[category] = (coef, intercept) 저장.
        """
    ...
    def _define_sets(self):
        """
        - 아이템 집합 P, NP, All 정의
        - 노드 집합 Fac, St 정의 (Wh 제거)
        - 전체 노드(AllNodes) = Fac ∪ St
        ...
        # 노드 집합 정의 (창고 WI_wh 제거)
        self.Fac = ["WI_fac"]
        self.St = ["WI_1", "WI_2", "WI_3"]

        self.AllNodes = self.Fac + self.St
    ...
    def _build_objective(self):
        ...
        Objective:
          ∑_{t}
            ...
            + ∑_{i∈AllItems} ∑_{(n₁,n₂)∈(Fac×St)}
                ( coef_i·distance[n₁,n₂]·x[i,n₁,n₂,t]
                + intercept_i·distance[n₁,n₂]·N[n₁,n₂,t] ) ]
        """
        # (1) 아크(arcs) 생성: Fac × St
        arcs = [(f, s) for f in self.Fac for s in self.St]
    ...
    def _build_constraints(self):
        ...
        # (A) 아크(arcs) 생성: Fac × St
        arcs = [(f, s) for f in Fac for s in St]
        ...
        for n1, n2 in arcs:
            for t in periods:
                ...
                if (
                    ...
                    # 생산지(origin) → 창고 노드 없이 바로 U_wh 적용
                    U_origin = self.U_wh
...
if __name__ == "__main__":
    ...
    # 4) “WI_wh” 노드(창고)가 모델에 포함되었는지 확인
    wh_vars = [v.varName for v in solver.model.getVars() if "WI_wh" in v.varName]
    wh_constrs = [
        c.ConstrName for c in solver.model.getConstrs() if "WI_wh" in c.ConstrName
    ]

    print("WI_wh 관련 변수:", wh_vars)
    print("WI_wh 관련 제약:", wh_constrs)
